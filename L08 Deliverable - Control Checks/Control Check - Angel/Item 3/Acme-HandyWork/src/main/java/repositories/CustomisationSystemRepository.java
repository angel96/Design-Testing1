...
public interface CustomisationSystemRepository extends JpaRepository<CustomisationSystem, Integer> {
	...
	//Control Check - Queries
	@Query("select (select count(q)*1.0 from Aolet q where q.finalMode = true)/count(f), 'null', 'null', sqrt((sum((select count(q)*1.0 from f.aolets q where q.finalMode = true)*(select count(q)*1.0 from f.aolets q where q.finalMode = true))/count(f)) - ((select count(q)*1.0 from Aolet q where q.finalMode = true)/count(f)* (select count(q)*1.0 from Aolet q where q.finalMode = true)/count(f)))  from FixUpTask f")
	String findAoletsPerFixUpTask();

	@Query("select (count(q)*1.0/(select count(q)*1.0 from Aolet q)) from Aolet q where q.finalMode = '1'")
	Double ratioPublishedAoletsPerFixUpTask();

	@Query("select (count(q)*1.0/(select count(q)*1.0 from Aolet q)) from Aolet q where q.finalMode = '0'")
	Double ratioUnPublishedAoletsPerFixUpTask();
