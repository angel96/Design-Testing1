...
public interface CustomisationSystemRepository extends JpaRepository<CustomisationSystem, Integer> {
	...
	//Control Check, Query 1
	@Query("select (select count(w)*1.0 from Wage w where w.finalMode = true)/count(f), 'null', 'null',sqrt((sum((select count(w)*1.0 from f.wages w where w.finalMode = true)*(select count(w)*1.0 from f.wages w where w.finalMode = true))/count(f)) - ((select count(w)*1.0 from Wage w where w.finalMode = true)/count(f) * (select count(w)*1.0 from Wage w where w.finalMode = true)/count(f)))  from FixUpTask f")
	String stdAndAvgOfPublishedWagesPerTotatFixUpTask();

	//Control Check, Query 2
	@Query("select (count(w)*1.0/(select count(w)*1.0 from Wage w)) from Wage w where w.finalMode = '1'")
	Double findRatioPublishWagesPerTotalWages();

	//Control Check, Query 3
	@Query("select (count(w)*1.0/(select count(w)*1.0 from Wage w)) from Wage w where w.finalMode = '0'")
	Double findRatioNotPublishWagesPerTotalWages();
}
