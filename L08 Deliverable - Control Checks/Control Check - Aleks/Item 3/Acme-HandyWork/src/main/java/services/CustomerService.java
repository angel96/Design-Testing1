...
public class CustomerService {
	...
	public Customer findByFixUp(final int id) {
		Customer c;
		c = this.customerRepository.findByFixUpTask(id);
		final Collection<Endorsement> endorsements = this.serviceEndorsement.findEndorsementsByActorReceived(c);
		if (endorsements.size() > 0)
			c.setScore(Utiles.homotheticalTransformation(this.serviceEndorsement.findEndorsementsByActorReceived(c)));
		else
			c.setScore(0.0);
