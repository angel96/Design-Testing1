...
public class PhaseController extends AbstractController {
	...
	@Autowired
	private PhasesService		servicePhase;
	...
	public ModelAndView list(@RequestParam final int fixuptask) {
		...
		if (Utiles.findAuthority(user.getAuthorities(), Authority.HANDY_WORKER)) {
			...
			result.addObject("ownfixup", fixuptaskHandy.contains(t));
		}
	...
	public ModelAndView create(@RequestParam final int fixuptask) {
		...
		if (Utiles.findAuthority(a.getAccount().getAuthorities(), Authority.HANDY_WORKER))
			result.addObject("requestURI", "phase/handyworker/edit.do?fixup=" + fixuptask);
	...
	public ModelAndView edit(@RequestParam final int id) {
		ModelAndView result;
		result = new ModelAndView("phase/edit");
		result.addObject("phase", this.servicePhase.findOne(id));
	...
	public ModelAndView submit(@RequestParam final int fixup, @Valid final Phase phase, final BindingResult binding) {
		ModelAndView result;
		if (binding.hasErrors()) {
			...
			try {
				final FixUpTask f = this.servicefixuptask.findOne(fixup);
